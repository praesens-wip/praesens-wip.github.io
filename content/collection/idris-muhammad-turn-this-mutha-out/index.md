+++
title = "Turn This Mutha Out"
date = 2025-12-25
draft = false
description = "Tracks recorded at Mediasound, December 1976, and Electric Lady, February 1977."

[album]
artist = "Idris Muhammad"
releaseYear = 1977
label = "Kudu"
catalogNumber = "KU-34"
genres = ["Jazz", "Funk / Soul", "Jazz-Funk", "Disco"]

[album.links]
spotify = "https://open.spotify.com/album/4PznnWq19AwatCG4lsyb5I"
bandcamp = ""
appleMusic = "https://music.apple.com/us/album/turn-this-mutha-out/1183434429"
youtube = "https://www.youtube.com/watch?v=0skigTyA4NE"

[[album.tracklist]]
side = "Side A"
tracks = [
  "A1. Could Heaven Ever Be Like This",
  "A2. Camby Bolongo",
  "A3. Turn This Mutha Out"
]

[[album.tracklist]]
side = "Side B"
tracks = [
  "B1. Tasty Cakes",
  "B2. Crab Apple",
  "B3. Moon Hymn",
  "B4. Say What"
]

[[album.credits]]
section = "Credits"
people = [
  "Carole Kowalchuk - Design [Album]",
  "Sib Chalawick - Design [Album]",
  "Idris Muhammad - Drums, Tom Tom",
  "Dave Palmer (2) - Engineer [Electric Lady]",
  "Dave Wittman - Engineer [Electric Lady]",
  "Joe Jorgensen - Engineer [Mediasound]",
  "White Gate - Photography By [Album]",
  "Dave Matthews (3) - Producer, Arranged By",
  "Dave Matthews (3) - Written-By",
  "Tony Sarafino - Written-By"
]
+++

Tracks recorded at Mediasound, December 1976, and Electric Lady, February 1977.